...
- 연역적 방식
	- CHIRPS 라는 알고리즘을 사용함
	- 우리는 높은 중요도 랜덤 경로 스니펫 모음(CHIRPS)[29]이라는 알고리듬을 사용했는데, 이 알고리듬은 대조적인 설명으로 풍부하고 정밀도와 적용 범위가 높은 규칙 기반 로컬 설명을 생성합니다[45]. CHIRPS는 다음과 같은 표 형식으로 예측 결과를 설명하는 규칙을 추출합니다. 각 행에는 임계값(높거나 낮음)에 대해 테스트된 지표가 포함되어 RF 분류에 가장 많이 기여합니다. "Contrast"라는 열은 표 행에서 고려된 지표를 규칙(반실례)에서 제외할 경우 정밀도가 얼마나 저하되는지를 보여줍니다. 마지막 열 "결정"에는 RF 분류 결과가 포함됩니다(그림 1 D, 연역적 설명 참조).
